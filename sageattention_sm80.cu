...
#include "sageattention_cp_async.cuh"
#include "sageattention_mma.cuh"
#include "sageattention_permuted_smem.cuh"
#include "sageattention_math.cuh"
#include "sageattention_reduction_utils.cuh"
#include "sageattention_numeric_conversion.cuh"
#include "sageattention_dispatch_utils.h"

#include "sageattention_attn_utils.cuh"
...
struct SageAttentionWorkspace {
  int8_t* q_int8 = nullptr;
  int8_t* k_int8 = nullptr;
  half* out_fp16 = nullptr;
  float* q_scale = nullptr;
  float* k_scale = nullptr;
  float* k_mean = nullptr;
  size_t qkv_capacity_elems = 0;
  size_t out_capacity_elems = 0;
  size_t q_scale_capacity = 0;
  size_t k_scale_capacity = 0;
  size_t k_mean_capacity = 0;

  ~SageAttentionWorkspace() {
    free_all();
  }

  void free_all() {
    if (q_int8) CUDA_CHECK(cudaFree(q_int8));
    if (k_int8) CUDA_CHECK(cudaFree(k_int8));
    if (out_fp16) CUDA_CHECK(cudaFree(out_fp16));
    if (q_scale) CUDA_CHECK(cudaFree(q_scale));
    if (k_scale) CUDA_CHECK(cudaFree(k_scale));
    if (k_mean) CUDA_CHECK(cudaFree(k_mean));
    q_int8 = nullptr;
    k_int8 = nullptr;
    out_fp16 = nullptr;
    q_scale = nullptr;
    k_scale = nullptr;
    k_mean = nullptr;
    qkv_capacity_elems = 0;
    out_capacity_elems = 0;
    q_scale_capacity = 0;
    k_scale_capacity = 0;
    k_mean_capacity = 0;
  }

  void ensure_qkv(size_t elems) {
    if (qkv_capacity_elems >= elems) return;
    if (q_int8) CUDA_CHECK(cudaFree(q_int8));
    if (k_int8) CUDA_CHECK(cudaFree(k_int8));
    CUDA_CHECK(cudaMalloc(&q_int8, elems * sizeof(int8_t)));
    CUDA_CHECK(cudaMalloc(&k_int8, elems * sizeof(int8_t)));
    qkv_capacity_elems = elems;
  }

  void ensure_out(size_t elems) {
    if (out_capacity_elems >= elems) return;
    if (out_fp16) CUDA_CHECK(cudaFree(out_fp16));
    CUDA_CHECK(cudaMalloc(&out_fp16, elems * sizeof(half)));
    out_capacity_elems = elems;
  }

  void ensure_q_scale(size_t elems) {
    if (q_scale_capacity >= elems) return;
    if (q_scale) CUDA_CHECK(cudaFree(q_scale));
    CUDA_CHECK(cudaMalloc(&q_scale, elems * sizeof(float)));
    q_scale_capacity = elems;
  }

  void ensure_k_scale(size_t elems) {
    if (k_scale_capacity >= elems) return;
    if (k_scale) CUDA_CHECK(cudaFree(k_scale));
    CUDA_CHECK(cudaMalloc(&k_scale, elems * sizeof(float)));
    k_scale_capacity = elems;
  }

  void ensure_k_mean(size_t elems) {
    if (k_mean_capacity >= elems) return;
    if (k_mean) CUDA_CHECK(cudaFree(k_mean));
    CUDA_CHECK(cudaMalloc(&k_mean, elems * sizeof(float)));
    k_mean_capacity = elems;
  }
};

static SageAttentionWorkspace& sageattention_workspace() {
  static SageAttentionWorkspace ws;
  return ws;
}

template<int HEAD_DIM>
__global__ void sageattention_k_mean_kernel(const half* __restrict__ k,
  ...
  float sum = 0.0f;
  const half* head_ptr = k + (int64_t)head * seq_len * HEAD_DIM;
  for (int s = 0; s < seq_len; s++) {
    sum += __half2float(head_ptr[(int64_t)s * HEAD_DIM + d]);
  }
  k_mean[head * HEAD_DIM + d] = sum / fmaxf((float)seq_len, 1.0f);
}

template<int HEAD_DIM>
__global__ void sageattention_query_per_thread_int8_kernel(
  ...
  int block128 = group / 32;
  int rem = group % 32;
  int warp_q = rem / 8;
  int lane_group = rem % 8;
  int token_base = block128 * 128 + warp_q * 32 + lane_group;
  const half* head_ptr = input + (int64_t)head * seq_len * HEAD_DIM;
  int8_t* head_out = output + (int64_t)head * seq_len * HEAD_DIM;

  float vals[4];
  float amax = 1e-7f;
#pragma unroll
  for (int i = 0; i < 4; i++) {
    int token = token_base + i * 8;
    float v = 0.0f;
    if (token < seq_len) {
      v = __half2float(head_ptr[(int64_t)token * HEAD_DIM + d]);
    }
    vals[i] = v;
  ...
  float block_amax = vllm::blockReduceMax(amax);
  if (threadIdx.x == 0) {
    block_scale = block_amax / 127.0f + 1e-7f;
    scale[(int64_t)head * (div_ceil(seq_len, 128) * 32) + block128 * 32 + warp_q * 8 + lane_group] = block_scale;
  }
  __syncthreads();
...
#pragma unroll
  for (int i = 0; i < 4; i++) {
    int token = token_base + i * 8;
    if (token < seq_len) {
      head_out[(int64_t)token * HEAD_DIM + d] = float_to_int8_rn(vals[i] * inv_scale);
    }
  }
}

template<int HEAD_DIM>
__global__ void sageattention_key_per_thread_int8_kernel(
    const half* __restrict__ input,
  ...
  int block64 = group / 4;
  int lane_group = group % 4;
  int token_base = block64 * 64 + lane_group * 2;
  const half* head_ptr = input + (int64_t)head * seq_len * HEAD_DIM;
  int8_t* head_out = output + (int64_t)head * seq_len * HEAD_DIM;
  float mean = k_mean[head * HEAD_DIM + d];

  float vals[16];
  float amax = 1e-7f;
#pragma unroll
  for (int i = 0; i < 8; i++) {
    int token0 = token_base + i * 8;
    int token1 = token0 + 1;
    float v0 = 0.0f;
    float v1 = 0.0f;
    if (token0 < seq_len) {
      v0 = __half2float(head_ptr[(int64_t)token0 * HEAD_DIM + d]) - mean;
    }
    if (token1 < seq_len) {
      v1 = __half2float(head_ptr[(int64_t)token1 * HEAD_DIM + d]) - mean;
    }
    vals[i * 2 + 0] = v0;
    vals[i * 2 + 1] = v1;
  ...
  if (threadIdx.x == 0) {
    block_scale = block_amax / 127.0f + 1e-7f;
    scale[(int64_t)head * (div_ceil(seq_len, 64) * 4) + block64 * 4 + lane_group] = block_scale;
  }
  __syncthreads();
...
#pragma unroll
  for (int i = 0; i < 8; i++) {
    ...
    int token1 = token0 + 1;
    if (token0 < seq_len) {
      head_out[(int64_t)token0 * HEAD_DIM + d] = float_to_int8_rn(vals[i * 2 + 0] * inv_scale);
    }
    if (token1 < seq_len) {
      head_out[(int64_t)token1 * HEAD_DIM + d] = float_to_int8_rn(vals[i * 2 + 1] * inv_scale);
    }
  }
}

__global__ void sageattention_half_to_float_kernel(const half* __restrict__ input,
...
void sageattention_launch_accum_f32(const int8_t* query,
                                    const int8_t* key,
                                    const half* value,
                                    half* output,
  ...
  size_t smem_max = std::max<size_t>(
  ...
  dim3 grid(div_ceil(seq_len, CTA_Q), n_heads, 1);
  dim3 block(32, (CTA_Q / WARP_Q) * (CTA_K / WARP_K));

  int stride_bz = n_heads * seq_len * HEAD_DIM;
  ...
  kernel_func<<<grid, block, smem_max, stream>>>(
      ...
      seq_len,
      1,
      stride_bz, stride_seq, stride_head,
      stride_bz, stride_seq, stride_head,
      stride_bz, stride_seq, stride_head,
      stride_bz, stride_seq, stride_head,
      sm_scale);
...
void attention_forward_sage_sm80_fp16io(const __half* q,
                                        const __half* k,
                                        const __half* v,
                                        float* out,
                                        float scale,
                                        ...
                                        cudaStream_t stream) {
  if (D != 64 && D != 128) {
    fprintf(stderr, "SageAttention SM80 only supports head_dim 64 or 128 (got %d)\n", D);
    exit(1);
  }

  SageAttentionWorkspace& ws = sageattention_workspace();
  size_t qkv_elems = (size_t)BH * S * D;
  size_t q_scale_elems = (size_t)BH * div_ceil(S, 128) * 32;
  size_t k_scale_elems = (size_t)BH * div_ceil(S, 64) * 4;
  size_t k_mean_elems = (size_t)BH * D;

  ws.ensure_qkv(qkv_elems);
  ws.ensure_out(qkv_elems);
  ws.ensure_q_scale(q_scale_elems);
  ws.ensure_k_scale(k_scale_elems);
  ws.ensure_k_mean(k_mean_elems);

  DISPATCH_HEAD_DIM(D, HEAD_DIM, {
    dim3 mean_grid(BH);
    dim3 mean_block(HEAD_DIM);
    sageattention_k_mean_kernel<HEAD_DIM><<<mean_grid, mean_block, 0, stream>>>(
        reinterpret_cast<const half*>(k), ws.k_mean, BH, S);

    dim3 q_grid(div_ceil(S, 128) * 32, BH);
    dim3 q_block(HEAD_DIM);
    sageattention_query_per_thread_int8_kernel<HEAD_DIM><<<q_grid, q_block, 0, stream>>>(
        reinterpret_cast<const half*>(q), ws.q_int8, ws.q_scale, BH, S);

    dim3 k_grid(div_ceil(S, 64) * 4, BH);
    dim3 k_block(HEAD_DIM);
    sageattention_key_per_thread_int8_kernel<HEAD_DIM><<<k_grid, k_block, 0, stream>>>(
        reinterpret_cast<const half*>(k), ws.k_mean, ws.k_int8, ws.k_scale, BH, S);

    DISPATCH_CAUSAL(causal ? 1 : 0, IS_CAUSAL, {
      sageattention_launch_accum_f32<HEAD_DIM, IS_CAUSAL>(
          ws.q_int8,
          ws.k_int8,
  ...
  int64_t n = (int64_t)qkv_elems;
  int block = 256;
  int grid = (int)((n + block - 1) / block);
  sageattention_half_to_float_kernel<<<grid, block, 0, stream>>>(ws.out_fp16, out, n);
}
