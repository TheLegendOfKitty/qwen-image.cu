...
Tensor transformer_forward(const TransformerWeights& w,
    ...
    // Reusable scratch for expanding BF16 weights into FP32 before GEMM.
    ...
    Tensor t_after_l1 = Tensor::alloc({1, inner_dim}, DType::FP32);
    {
        linear_forward_fp32in_bf16w_fp32out(t_emb_sin, w.time_linear1_weight, &w.time_linear1_bias,
                                            t_after_l1, weight_fp32_scratch);
    }
    ...
    Tensor t_emb = Tensor::alloc({1, inner_dim}, DType::FP32);
    {
        linear_forward_fp32in_bf16w_fp32out(t_after_l1, w.time_linear2_weight, &w.time_linear2_bias,
                                            t_emb, weight_fp32_scratch);
    }
    t_after_l1.free_data();

    // Debug dump infrastructure - only dump on first call
    static int call_count = 0;
    call_count++;
    auto dump = [](const char* path, const void* gpu_data, int64_t n, bool is_bf16) {
        if (!getenv("DUMP_TRANSFORMER") || call_count != 1) return;
        ...
        fprintf(stderr, "  DUMP: %s [%ld] mean=%.6f std=%.4f first5: %.6f %.6f %.6f %.6f %.6f\n",
                path, (long)n, mean, std_val, f32[0], f32[1], f32[2], f32[3], f32[4]);
    };
    ...
    Tensor img = Tensor::alloc({n_img, inner_dim}, DType::FP32);
    {
        ...
        linear_forward_fp32out(patches_2d, w.img_in_weight, &w.img_in_bias, img_2d);
    }
    ...
    Tensor txt = Tensor::alloc({n_txt, inner_dim}, DType::FP32);
    {
        ...
        linear_forward_fp32out(tn_2d, w.txt_in_weight, &w.txt_in_bias, txt_2d);
    }
    ...
    for (int bi = 0; bi < 60; bi++) {
        ...
        if (bi == 0) {
            dump("cuda_b0_img_mods.bin", img_mods, 6 * inner_dim, false);
            dump("cuda_b0_txt_mods.bin", txt_mods, 6 * inner_dim, false);
        }
        ...
        if (bi == 0) dump("cuda_b0_img_ln.bin", img_normed_fp32.data, (int64_t)n_img * inner_dim, false);
        ...
        if (bi == 0) dump("cuda_b0_img_modulated.bin", img_normed_fp32.data, (int64_t)n_img * inner_dim, false);
        ...
        // Attention projections: FP32 activations, FP32 GEMM, BF16 weights expanded on demand.
        {
            ...
            linear_forward_fp32in_bf16w_fp32out(img_normed_fp32, b.to_q_weight, &b.to_q_bias, oq, weight_fp32_scratch);
            linear_forward_fp32in_bf16w_fp32out(img_normed_fp32, b.to_k_weight, &b.to_k_bias, ok, weight_fp32_scratch);
            linear_forward_fp32in_bf16w_fp32out(img_normed_fp32, b.to_v_weight, &b.to_v_bias, ov, weight_fp32_scratch);
        }
        {
            Tensor oq = txt_q.view({n_txt, inner_dim});
            Tensor ok = txt_k.view({n_txt, inner_dim});
            Tensor ov = txt_v.view({n_txt, inner_dim});
            linear_forward_fp32in_bf16w_fp32out(txt_normed_fp32, b.add_q_proj_weight, &b.add_q_proj_bias, oq, weight_fp32_scratch);
            linear_forward_fp32in_bf16w_fp32out(txt_normed_fp32, b.add_k_proj_weight, &b.add_k_proj_bias, ok, weight_fp32_scratch);
            linear_forward_fp32in_bf16w_fp32out(txt_normed_fp32, b.add_v_proj_weight, &b.add_v_proj_bias, ov, weight_fp32_scratch);
        }

        if (bi == 0) {
            dump("cuda_b0_img_q_raw.bin", img_q.data, (int64_t)n_img * inner_dim, false);
            dump("cuda_b0_img_k_raw.bin", img_k.data, (int64_t)n_img * inner_dim, false);
            dump("cuda_b0_img_v_raw.bin", img_v.data, (int64_t)n_img * inner_dim, false);
        }
        ...
        if (bi == 0) {
            dump("cuda_b0_img_q_normed.bin", img_q.data, (int64_t)n_img * inner_dim, false);
            dump("cuda_b0_img_k_normed.bin", img_k.data, (int64_t)n_img * inner_dim, false);
            dump("cuda_b0_txt_q_normed.bin", txt_q.data, (int64_t)n_txt * inner_dim, false);
        }
        ...
        if (bi == 0) {
            dump("cuda_b0_q_joint.bin", q_t.data, (int64_t)n_heads * total_seq * head_dim, false);
            dump("cuda_b0_k_joint.bin", k_t.data, (int64_t)n_heads * total_seq * head_dim, false);
        }
        ...
        if (bi == 0) {
            dump("cuda_b0_q_after_rope.bin", q_t.data, (int64_t)n_heads * total_seq * head_dim, false);
            dump("cuda_b0_k_after_rope.bin", k_t.data, (int64_t)n_heads * total_seq * head_dim, false);
            dump("cuda_b0_v_joint.bin", v_t.data, (int64_t)n_heads * total_seq * head_dim, false);
        }
        ...
        if (bi == 0) {
            dump("cuda_b0_attn_out_img.bin", img_attn_out.data, (int64_t)n_img * inner_dim, false);
            dump("cuda_b0_attn_out_txt.bin", txt_attn_out.data, (int64_t)n_txt * inner_dim, false);
        }

        // Output projections stay FP32 on the activation side.
        {
            Tensor ip_2d = img_proj.view({n_img, inner_dim});
            linear_forward_fp32in_bf16w_fp32out(img_attn_out, b.to_out_weight, &b.to_out_bias, ip_2d, weight_fp32_scratch);

            Tensor tp_2d = txt_proj.view({n_txt, inner_dim});
            linear_forward_fp32in_bf16w_fp32out(txt_attn_out, b.to_add_out_weight, &b.to_add_out_bias, tp_2d, weight_fp32_scratch);
        }

        if (bi == 0) {
            dump("cuda_b0_img_proj.bin", img_proj.data, (int64_t)n_img * inner_dim, false);
        }
        ...
        if (bi == 0) {
            dump("cuda_b0_img_after_attn.bin", img.data, (int64_t)n_img * inner_dim, false);
        }
        ...
        // GELU MLP
        if (bi == 0) dump("cuda_b0_img_mlp_input.bin", img_normed_fp32.data, (int64_t)n_img * inner_dim, false);
        {
            Tensor mlp_2d = img_mlp_fp32.view({n_img, mlp_dim});
            linear_forward_fp32in_bf16w_fp32out(img_normed_fp32, b.img_mlp_fc1_weight, &b.img_mlp_fc1_bias, mlp_2d, weight_fp32_scratch);
            if (bi == 0) dump("cuda_b0_img_mlp_fc1.bin", img_mlp_fp32.data, (int64_t)n_img * mlp_dim, false);
            // GELU in FP32
            gelu_fp32((float*)img_mlp_fp32.data, (float*)img_mlp_fp32.data,
                      (int64_t)n_img * mlp_dim);
            if (bi == 0) dump("cuda_b0_img_mlp_gelu.bin", img_mlp_fp32.data, (int64_t)n_img * mlp_dim, false);
            Tensor out_2d = img_mlp_out.view({n_img, inner_dim});
            linear_forward_fp32in_bf16w_fp32out(img_mlp_fp32, b.img_mlp_fc2_weight, &b.img_mlp_fc2_bias, out_2d, weight_fp32_scratch);
            if (bi == 0) dump("cuda_b0_img_mlp_out.bin", img_mlp_out.data, (int64_t)n_img * inner_dim, false);
        }
        {
            Tensor mlp_2d = txt_mlp_fp32.view({n_txt, mlp_dim});
            linear_forward_fp32in_bf16w_fp32out(txt_normed_fp32, b.txt_mlp_fc1_weight, &b.txt_mlp_fc1_bias, mlp_2d, weight_fp32_scratch);
            ...
            linear_forward_fp32in_bf16w_fp32out(txt_mlp_fp32, b.txt_mlp_fc2_weight, &b.txt_mlp_fc2_bias, out_2d, weight_fp32_scratch);
        }
        ...
        if (bi == 0) {
            dump("cuda_b0_img_after_mlp.bin", img.data, (int64_t)n_img * inner_dim, false);
            dump("cuda_b0_txt_after_mlp.bin", txt.data, (int64_t)n_txt * inner_dim, false);
        }
    ...
    linear_forward_fp32in_bf16w_fp32out(img_normed_fp32, w.proj_out_weight, &w.proj_out_bias,
                                        img_proj_out_fp32, weight_fp32_scratch);
